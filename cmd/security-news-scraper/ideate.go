...
import (
	...
	"github.com/Harsh-Sonker/security-news-scraper/internal/ai"
	"github.com/Harsh-Sonker/security-news-scraper/internal/rank"
...
func runIdeate(cmd *cobra.Command, args []string) error {
	...
	if !aiCfg.Enabled {
		if !isInteractive(cmd) {
			return fmt.Errorf("AI is not set up — run `security-news-scraper ai` to configure a provider")
		}
		...
		if aiCfg.Provider == ai.ProviderQwen && !setup.OllamaReachable(aiCfg.Qwen.BaseURL) {
			fmt.Fprintln(cmd.OutOrStdout(), "Ollama isn't reachable yet — finish the steps above, then run: security-news-scraper ideate")
			return nil
		}
	}
