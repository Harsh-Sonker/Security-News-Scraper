...
# install.sh
#
# One-shot installer for security-news-scraper. Takes a fresh machine to `security-news-scraper` runnable
...
# CONFIG
# ============================================================================
REPO_OWNER="HarshSonker"
REPO_NAME="security-news-scraper"
BINARY="security-news-scraper"
TAGLINE="Security news + CVE intelligence in your terminal. Keyless by default."
REPO_URL="https://github.com/${REPO_OWNER}/${REPO_NAME}.git"
INSTALL_DIR="${SECURITY_NEWS_SCRAPER_INSTALL_DIR:-$HOME/.local/bin}"
DEFAULT_BRANCH="main"
GO_MIN="1.21"
PREBUILT=1
...
install.sh — install ${BINARY}
  ...
  ./install.sh [options]
  curl -fsSL https://example.com/${BINARY}/install.sh | bash
...
build_from_source() {
    ...
    mkdir -p "$INSTALL_DIR"
    GOBIN="$INSTALL_DIR" go install ./cmd/security-news-scraper || die "go install failed"
