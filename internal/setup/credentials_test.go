...
func TestLoadEnvWins(t *testing.T) {
	t.Setenv("XDG_CONFIG_HOME", t.TempDir())
	if err := Save(map[string]string{"OPENAI_API_KEY": "fromfile", "SECURITY_NEWS_SCRAPER_TEST_ONLY": "x"}); err != nil {
		t.Fatal(err)
	}
	t.Setenv("OPENAI_API_KEY", "fromenv")
	defer os.Unsetenv("SECURITY_NEWS_SCRAPER_TEST_ONLY")
	...
	if got := os.Getenv("SECURITY_NEWS_SCRAPER_TEST_ONLY"); got != "x" {
		t.Errorf("SECURITY_NEWS_SCRAPER_TEST_ONLY = %q, want x (unset var loads from file)", got)
	}
}
