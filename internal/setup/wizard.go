...
func Run(in io.Reader, out io.Writer) error {
	fmt.Fprintln(out, "security-news-scraper AI setup")
...
func setupAPIProvider(in io.Reader, out io.Writer, c apiChoice) error {
	...
	fmt.Fprintf(out, "\nSaved to %s (0600). %s is ready — run:  security-news-scraper ideate\n", path, c.label)
	return nil
}
...
func setupOllama(out io.Writer) error {
	...
	case HasBinary("docker"):
		...
		fmt.Fprintln(out, "(first run pulls qwen2.5:7b, a few minutes), then re-run `security-news-scraper ai` and pick 4.")
	...
	default:
		fmt.Fprintf(out, "Install Ollama (%s) or Docker, then re-run `security-news-scraper ai`.\n", ollamaInstall)
		return saveOllama(out, composeQwenURL)
	}
}
...
func saveOllama(out io.Writer, baseURL string) error {
	...
	path, _ := CredentialsPath()
	fmt.Fprintf(out, "Saved qwen (%s) to %s. Run:  security-news-scraper ideate\n", baseURL, path)
	return nil
}
