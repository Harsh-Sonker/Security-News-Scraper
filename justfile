...
build:
    go build -o {{binary}} ./cmd/security-news-scraper

run *args:
    go run ./cmd/security-news-scraper {{args}}
...
watch *args:
    go run ./cmd/security-news-scraper watch {{args}}

publish target="~/dev/security-news-scraper":
    rsync -a --delete \
