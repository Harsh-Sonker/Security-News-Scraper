...
# Security-news-scraper: Overview
...
The signal is usually some combination of the same factors. A vulnerability that many outlets picked up in the same few hours is trending for a reason. A CVE that CISA just added to its Known Exploited Vulnerabilities catalog is being used against real targets today. A flaw with a high EPSS score is one the rest of the world is about to start exploiting. Security-news-scraper exists to compute that combination for you and sort by it.
...
curl -fsSL https://example.com/security-news-scraper/install.sh | bash
# or, with a Go toolchain:
go install github.com/Harsh-Sonker/security-news-scraper/cmd/security-news-scraper@latest

# Pull the latest news, cluster it, and enrich every CVE:
security-news-scraper scrape

# Browse the ranked dossier:
security-news-scraper tui
...
├── cmd/security-news-scraper/          # the CLI: one file per command, plus the shared pipeline seam
├── internal/
...
The single most important thing to understand first is the pipeline in `cmd/security-news-scraper/pipeline.go` and `scrape.go`. Everything in `internal` exists to feed one function, `ingestAndCluster`, and everything downstream exists to rank and present what it produced.
...
```
Solution: one slow or broken feed never aborts a run. The other sources still ingest, the failed one is reported at the bottom, and the next scrape retries it. A persistent failure usually means the feed URL moved. Check `security-news-scraper sources`.

**`tui` says there is nothing to show**
```
no clusters yet. run: security-news-scraper scrape
```
Solution: the TUI reads what is already in the store. Run `security-news-scraper scrape` once first. Scrape enriches automatically, so there is no separate step.

**`ideate` says AI is not configured**
Solution: the AI layer is opt in and off by default. Run `security-news-scraper ai` to set up a provider (paste one key, or point it at a local Ollama). The aggregator is fully useful with AI disabled.
