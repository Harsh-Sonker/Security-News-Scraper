...
# Security-news-scraper: Architecture
...
Orchestration does not live inside any leaf package. It lives in `cmd/security-news-scraper`, and the shared core of it is one function, `ingestAndCluster` in `pipeline.go`, which both `scrape` and `watch` call. That is the seam that keeps the two commands from drifting: when a step is added to the pipeline, both paths get it because there is only one place to add it.
