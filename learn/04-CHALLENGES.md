...
# Security-news-scraper: Challenges
...
**Add a source.** The source list is `sources.yaml`, embedded by default. Add an entry with a name, feed URL, type, and weight, then run `security-news-scraper sources` to persist it and `security-news-scraper scrape` to pull it. Pick a feed that overlaps with the existing seven (say, a second general security outlet) and watch it start joining clusters. The source `weight` feeds ranking, so a source you trust more nudges its stories up.

**Retune the ranking.** Every weight lives in `config`, under `rank.weights`. The defaults are news first (recency and velocity are the heaviest weights). Flip that: crank `cvss`, `kev`, and `epss` up and `recency` down, scrape, and compare the top of `security-news-scraper digest`. You will see the list reorder toward raw severity. There is no code change here, which is the point: the model is data, not logic.
...
**Add an export format.** `internal/export` renders Markdown and JSON. Add a third, for instance a plain text digest or an RSS feed of the ranked clusters, so security-news-scraper can feed another tool. Keep the renderer a pure function of the scored clusters, the way `export.Markdown` is, so it stays testable.
...
**Report per source health.** `fetch_state` already stores each source's last status and last fetch time. Add a `security-news-scraper sources --health` view that flags a source that has returned an error or a 304 for many runs in a row, so a silently dead feed becomes visible.
...
**Add full text search.** SQLite ships FTS5. Add a virtual table over article titles and bodies and a `security-news-scraper search "term"` command, so a user can search the whole archive by product, whether or not it is currently trending. Mind the migration: it is a new table, applied forward only, the way `internal/store` applies every other schema change.
...
**Ship a diff.** Add a `security-news-scraper digest --since-last` that shows only what changed since the previous digest: new clusters, clusters that grew, and CVEs that newly landed on KEV. This is the report a person actually wants each morning, and it forces you to think about what "changed" means for a cluster, which is the same watermark problem the watch daemon solves.
...
- **sbom-generator-vulnerability-matcher** answers the question this project raises. Security-news-scraper tells you a CVE is trending and exploited. The SBOM matcher tells you whether that CVE is in something you actually ship. Together they are a prioritization loop.
